...
public class Handlers {
    ...
    /**
     * To be put on classes that are Forge/FML event handlers. If you are using this from another mod, be sure to
     * implement {@link IModTT} on your
     * {@code @Mod} class, or call {@code Handlers.addPackage("your.base.package")} so that this class can search your
     * classes
     * <p>
        ...
        for (String packageName : packageSet) {
            ...
            for (ClassInfo info : classes) {
                ...
                if (!info.getPackageName()
                    .contains("client") || FMLCommonHandler.instance()
                        .getEffectiveSide()
                        .isClient()) {
                    try {
                        Class<?> c = info.load();
                        ...
                        TTCore.logger
                            .error(String.format("[Handlers] %s threw an error on load, skipping...", info.getName()));
                        t.printStackTrace();
                    }
                } else {
                    TTCore.logger.info(
                        String.format(
                            "[Handlers] Skipping client class %s, we are on a dedicated server",
                            info.getSimpleName()));
                }
            }
        }
    ...
    private void registerHandler(Class<?> c, Handler handler) throws InstantiationException, IllegalAccessException {
        ...
        if (ArrayUtils.contains(types, HandlerType.FORGE)) MinecraftForge.EVENT_BUS.register(inst);

        if (ArrayUtils.contains(types, HandlerType.FML)) FMLCommonHandler.instance()
            .bus()
            .register(inst);
    }
    ...
    private Object tryInit(Handler annot, Class<?> c) {
        ...
        throw new RuntimeException(
            "Could not instantiate @Handler class " + c.getName() + " or access INSTANCE field or instance() method.");
    }
}
