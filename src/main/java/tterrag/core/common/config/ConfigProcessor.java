...
public class ConfigProcessor {
     ...
     * not pass a handler that has not
            ...
            switch (adapter.getType()) {
                ...
                case INTEGER:
                    if (defVal.getClass()
                        ...
                        res = AbstractConfigHandler
                            .boundValue(prop, Bound.of(bound.min.intValue(), bound.max.intValue()), (Integer) defVal);
                    }
                    break;
                case STRING:
                    if (defVal.getClass()
    ...
    private ITypeAdapter getAdapterFor(Field f) {
        ...
        for (ITypeAdapter adapter : adapters) {
            if ((c.isPrimitive() && c == adapter.getPrimitiveType()) || adapter.getActualType()
                .isAssignableFrom(t)) {
                return adapter;
            }
        }
